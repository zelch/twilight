...
void SV_SendServerinfo (client_t *client)
{
	...
	sprintf (message, "%c\nLordHavoc NQ-SERVER VERSION %4.2f SERVER (%i CRC)", 2, VERSION, pr_crc);
...
// send over all entities (except the client) that touch the pvs
	...
	for (e=1 ; e<sv.num_edicts ; e++, ent = NEXT_EDICT(ent))
	{
		...
		if (ent != clent)
		{
			...
			{
				if (sv.models[ (int)ent->v.modelindex ]->flags == 0 && (ent->v.effects == EF_NODRAW))
					continue;
			}
...
//			VectorCopy(ent->v.origin, origin);
		...
		if ((int)(origin[0]*8.0) != (int)(ent->baseline.origin[0]*8.0))						bits |= U_ORIGIN1;
		if ((int)(origin[1]*8.0) != (int)(ent->baseline.origin[1]*8.0))						bits |= U_ORIGIN2;
		if ((int)(origin[2]*8.0) != (int)(ent->baseline.origin[2]*8.0))						bits |= U_ORIGIN3;
		if ((int)(angles[0]*(256.0/360.0)) != (int)(ent->baseline.angles[0]*(256.0/360.0)))	bits |= U_ANGLE1;
		if ((int)(angles[1]*(256.0/360.0)) != (int)(ent->baseline.angles[1]*(256.0/360.0)))	bits |= U_ANGLE2;
		if ((int)(angles[2]*(256.0/360.0)) != (int)(ent->baseline.angles[2]*(256.0/360.0)))	bits |= U_ANGLE3;
		if (ent->baseline.colormap != (int) ent->v.colormap)								bits |= U_COLORMAP;
		if (ent->baseline.skin != (int) ent->v.skin)										bits |= U_SKIN;
		if ((ent->baseline.frame & 0x00FF) != ((int) ent->v.frame & 0x00FF))				bits |= U_FRAME;
		if ((ent->baseline.effects & 0x00FF) != ((int) ent->v.effects & 0x00FF))			bits |= U_EFFECTS;
		if (ent->baseline.modelindex != (int) ent->v.modelindex)							bits |= U_MODEL;
