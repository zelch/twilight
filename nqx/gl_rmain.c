...
R_DrawSubSkin (aliashdr_t *paliashdr, skin_sub_t *skin, vec4_t *color)
{
	...
	for (i = 0; i < (unsigned)skin->num_indices; i++)
	{
		if ((unsigned)skin->indices[i] >= (unsigned)paliashdr->numverts)
			Host_Error ("%s has been corrupted, wants vert %i of %i\n",
					currententity->model->name, skin->indices[i],
					paliashdr->numverts);
	}
