...
{
	int passdatawidth[WAVEBUFFERS], passdataheight[WAVEBUFFERS], i, a, b, x, y, astart, bstart, wsize, hsize, width, height, steps, pos;
	float *passdata[WAVEBUFFERS], o0r, o0g, o0b, o1r, o1g, o1b, o2r, o2g, o2b, o3r, o3g, o3b, *in, *out, *row, *pix, *w, w0, w1, w2, w3;
	int failed;
	...
	if (!failed)
	{
		...
		for (steps = WAVESTEPS;steps;steps--)
		{
			...
			in = passdata[steps];
			for (y = -WAVELETPREFETCH;y < height;y += 2)
			{
				hsize = height - y;
				if (hsize > WAVESIZE)
					hsize = WAVESIZE;
				...
				for (x = -WAVELETPREFETCH;x < width;x += 2)
				{
					...
					if (o1r || o1g || o1b)
					{
						if (o2r || o2g || o2b)
						{
							if (o3r || o3g || o3b)
							{
								for (b = bstart, pix = row + (x + astart) * 3;b < hsize;b++, pix += width * 3)
								{
									w = &wave[b][astart][0];
