...
static int intwave[4][WAVESIZE][WAVESIZE];
...
static void waveletsetup(void)
{
	...
	for (y = 0;y < WAVESIZE;y++)
	{
		...
		{
			intwave[0][y][x] = (wavelet[               x] * wavelet[               y]) * 256.0;
			intwave[1][y][x] = (wavelet[WAVESIZE - 1 - x] * wavelet[               y]) * (((x    ) & 1) ? -1.0 : 1.0) * 256.0;
			intwave[2][y][x] = (wavelet[               x] * wavelet[WAVESIZE - 1 - y]) * (((y    ) & 1) ? -1.0 : 1.0) * 256.0;
			intwave[3][y][x] = (wavelet[WAVESIZE - 1 - x] * wavelet[WAVESIZE - 1 - y]) * (((x + y) & 1) ? -1.0 : 1.0) * 256.0;
		}
	}
}
...
static int dpi_decompressimage(dpiinfo_t *dpiinfo, hz_bitstream_readblocks_t *blocks)
{
	...
	int *passdata[WAVEBUFFERS], o, *in, *out, valuescale, *row, *pix;
	int failed;
#if HUFFMAN
